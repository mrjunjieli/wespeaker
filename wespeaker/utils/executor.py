...
def run_epoch(dataloader, epoch_iter, model, criterion, optimizer, scheduler,
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast(enabled=configs['enable_amp']):
            ...
            if configs['model'].startswith('PLUS_XI_VEC'):
                if i==0:
                    model.module.uttemb={}
                if epoch>EPOCH:
                    ...
                    mse_loss = mse(var_diag**0.5, ground_var)
            ...
            if epoch>EPOCH and configs['model'].startswith('PLUS_XI_VEC'):
                alpha = (100-epoch)/50 if epoch <100 else 0
                loss = loss + 0.5*(1-alpha)* mse_loss
