...
Same as :mod:`wespeaker.bin.score`, but the embedding covariance dumped by
:mod:`wespeaker.bin.extract_uncertainty` (``xvector_variance.scp``) is used
to rescale the cosine similarity:

    s = alpha_1 * alpha_2 * cos(e1, e2)
    alpha = sqrt( (e.e) / sum(e / (1 + var) * e) )

so that an utterance with a large estimated variance contributes a smaller
score magnitude.
...
def uncertainty_scale(emb, var):
    """alpha = sqrt( (e.e) / sum(e / (1 + var) * e) )."""
    numerator = np.dot(emb, emb)
    denominator = np.sum(emb / (1.0 + var) * emb)
...
def trials_cosine_score(eval_scp_path='',
    ...
    # the diagonal covariance lives next to the embeddings
    var_scp_path = os.path.join(os.path.dirname(eval_scp_path),
                                'xvector_variance.scp')
    if not os.path.exists(var_scp_path):
        raise FileNotFoundError(
            '{} not found, run wespeaker/bin/extract_uncertainty.py '
            'first'.format(var_scp_path))
    var_dict = {}
    ...
    for trial in trials:
        ...
        with open(trial, 'r') as trial_r, open(store_path, 'w') as w_f:
            ...
            for line in tqdm(lines,
                ...
                var1, var2 = var_dict[segs[0]], var_dict[segs[1]]

                alpha_1 = uncertainty_scale(emb1, var1)
                alpha_2 = uncertainty_scale(emb2, var2)
