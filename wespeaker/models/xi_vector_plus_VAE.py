...
class XI_VEC_PLUS_ECAPA_TDNN(nn.Module):
    ...
    def forward(self, x):
        ...
        #     out = self.bn2(out)
        # if train==True:
        #     z = self.reparameterize(out,var_diag)
        # else:
        z = out 

        return var_diag, z # (Batch, emb_dim)
...
def VAE_XI_VEC_ECAPA_TDNN_c1024(feat_dim, embed_dim, emb_bn=False):
...
def VAE_XI_VEC_ECAPA_TDNN_c512(feat_dim, embed_dim, emb_bn=False):
